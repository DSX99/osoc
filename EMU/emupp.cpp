...
decoder_ctrl decode(uint32_t opcode)
{
    ...
    */
    ...
    case 0b0010011: // math opers with imms
        ...
        case 0b101: // SRI
            if (!(imm & 0x20))
                ret_alu = SRL + USEIMM; // SRLI
            else
                ret_alu = SRA + USEIMM; // SRAI
            break;
        default:
            error += ILL_opcode;
        }
...
uint64_t alu(uint32_t opcode, uint32_t reg_1, uint32_t reg_2, uint32_t pc, uint32_t imm)
{
    ...
    */
    ...
    case BLT:
        out = reg_1 + reg_2;
        if ((int32_t)reg_1_val < (int32_t)reg_2_val)
            ctrl = 1;
        else
            ctrl = 0;
        break;
    ...
    case BGEU:
        ...
        break;
    case SLT:
        if ((int32_t)reg_1 < (int32_t)reg_2)
            out = 1;
        else
            out = 0;
        break;
    ...
    case AND:
        ...
        break;
    case SLL:
        out = reg_1 << (reg_2 & 0x1F);
        break;
    case SRL:
        out = reg_1 >> (reg_2 & 0x1F);
        break;
    case SRA:
        out = (int32_t)reg_1 >> (reg_2 & 0x1F);
        break;
    }
...
void sCPU::mem_intf(uint8_t lsu_act, uint32_t addr, uint32_t rd)
{
    ...
    if (addr > ROM_OFFSET)
    {
        ...
        case 0:
            ...
            regs_[rd] = mem_[addr - ROM_OFFSET] + (((mem_[addr - ROM_OFFSET] & 0x80) >> 7) * (0xffffff00));
            break;
        case 1:
            regs_[rd] = mem_[addr - ROM_OFFSET] + (mem_[addr - ROM_OFFSET + 1] << 8) + (((mem_[addr - ROM_OFFSET + 1] & 0x80) >> 7) * (0xffff0000));
            break;
        case 2:
            regs_[rd] = mem_[addr - ROM_OFFSET] + (mem_[addr - ROM_OFFSET + 1] << 8) + (mem_[addr - ROM_OFFSET + 2] << 16) + (mem_[addr - ROM_OFFSET + 3] << 24);
            break;
        case 3:
            regs_[rd] = mem_[addr - ROM_OFFSET];
            break;
        case 4:
            regs_[rd] = mem_[addr - ROM_OFFSET] + (mem_[addr - ROM_OFFSET + 1] << 8);
            break;
        case 5:
...
std::string decode_to_str(uint32_t opcode)
{
    ...
    */
    ...
    if (opcode == 0b00000000000100000000000001110011)
        return "EBREAK";
    ...
    if (operation == 0b0100011)
    { // Stores: sb rs2, imm(rs1)
        std::snprintf(buf, sizeof(buf), "x%d, %d(x%d)", rs2, ret_imm, rs1);
        out_assembly += buf;
    }
    else if (operation == 0b0000011 || operation == 0b1100111)
    { // Loads & JALR: lw rd, imm(rs1)
        std::snprintf(buf, sizeof(buf), "x%d, %d(x%d)", rd, ret_imm, rs1);
        out_assembly += buf;
    }
    else
    { // Generic R/I/U/J Types
        if (use_rd)
        {
...
{
    uint32_t code = 0, imm, aluop, rs1, rs2, idk, rd, value, lsu_act, jump, error;
    uint64_t recv_alu;
    ...
    {
        if (lsu_act == 5 || lsu_act == 6 || lsu_act == 7)
            rd = rs2;
