module ysyx_26060184_func (
    ...
    always_ff @(posedge clock) begin
        if(reset) begin
            ...
            if((match_ar) && (carvalid && carready))   trans <= 2'd2;
            if((match_aw) && (cawvalid && cawready))   trans <= 2'd1;
            
            if(trans[1] && crready && crvalid)         trans <= 2'b0;
            if(trans[0] && core_bready && core_bvalid) trans <= 2'b0;
        end
    end

    ysyx_26060184_core core (
        .clock(clock),
        .reset(reset),
    ...
    axi_slave_lsu ysyx_26060184_axi_slave_lsu_mod (
        .clk(clock), 
        .rst(reset), 
