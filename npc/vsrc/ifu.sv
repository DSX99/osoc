...
assign rready_ifu = !external_stall; // we are ready to accept new opcode iff lsu is not busy (we must keep opcode so result of lsu can be saved)
assign stall = (arvalid_ifu && !arready_ifu) || !rvalid_ifu; // we stall if we wait to accept addr or wait for data
