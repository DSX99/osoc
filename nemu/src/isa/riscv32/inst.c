...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 001 ????? 00100 11", sll   , R, R(rd) = src1 << BITS(src2, 4,0));

  INSTPAT("??????? ????? ????? 100 ????? 01100 11", xor    , R, R(rd) = src1 ^ src2);
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srl   , R, R(rd) = (word_t)src1 >> BITS(src2, 4,0));
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srl   , R, R(rd) = (sword_t)src1 >> BITS(src2, 4,0));
