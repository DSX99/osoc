...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 001 ????? 01100 11", sll    , R, R(rd) = src1 << BITS(src2, 4,0));
  INSTPAT("??????? ????? ????? 010 ????? 01100 11", slt    , R, R(rd) = (sword_t)src1 < (sword_t)src1);
  INSTPAT("??????? ????? ????? 011 ????? 01100 11", sltu   , R, R(rd) = (word_t)src1 < (word_t)src2);
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, if(src2==0){ R(rd) = UINT32_MAX; }else if(src1==(1<<31) && src2==UINT32_MAX){ R(rd) = src1; }else{ R(rd) = BITS((sword_t)src1 / (sword_t)src2, 31, 0); });
  INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu   , R, if(src2==0){ R(rd) = UINT32_MAX; }else{ R(rd) = BITS((sword_t)src1 / (sword_t)src2, 31, 0); } );
  ...
  INSTPAT("0000001 ????? ????? 111 ????? 01100 11", remu   , R, if(src2==0){ R(rd) = src1;      }else{ R(rd) = (sword_t)src1 % (sword_t)src2; } );
